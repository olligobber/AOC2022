...
data MoveState = MoveState [Position] deriving (Eq, Ord)

type Log = Set Position

moveState :: Direction -> State MoveState Log
moveState dir = do
	MoveState oldPos <- St.get
	let (oldHead:oldTails) = oldPos
	let newHead = moveHead dir oldHead
	let newPos = scanl moveTail newHead oldTails
	St.put $ MoveState newPos
	pure $ Se.singleton $ last newPos

startState :: MoveState
startState = MoveState $ replicate 10 startPos
